...
export const createEntityTool = defineTool({
  ...
  description:
    'Unified constructor for object / tag / material / shader. Handles parent linking, optional params, positions, and container slot assignment in one call. Returns the handle of the created entity (object handles include `path` for stable re-resolution) so you can chain set_params / set_keyframe. Note: `kind:"shader"` targets classical shader chains (Fusion, Colorizer, Xbitmap, …). For node-material edits (Standard node space / Redshift), use `apply_graph_description` instead.',
  inputShape: {
    kind: z.enum(["object", "tag", "material", "shader"]).describe("Entity kind to create."),
    type_id: z
      .union([z.number().int(), z.string()])
      .describe(
        "Plugin id. Accepts a numeric id (c4d.Ocube=5159, c4d.Ttexture=5616, …) or, for kind='object', an alias string: 'cube', 'sphere', 'cylinder', 'cone', 'torus', 'plane', 'disc', 'pyramid', 'platonic', 'null'.",
      ),
    name: z.string().optional().describe("Optional display name."),
    parent: handleSchema
      .optional()
      .describe(
        "Parent handle. Required for tag (owner object) and shader (owner VideoPost / tag / material). Optional for object (creates at scene root if omitted).",
      ),
    params: z
